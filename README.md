[![Build status](https://ci.appveyor.com/api/projects/status/l98m2aa87ndl8d34?svg=true)](https://ci.appveyor.com/project/persikfloro/postmanecho)
...
Пришлите на проверку ссылку на ваш репозиторий (удостоверьтесь, что в истории сборки были как Success, так и Fail, иначе будет не видно, как вы проверяли, что сборка падает в CI).
 ...
 ## Решение
 Подключен Continious Integration через сервис [AppVeyor](https://github.com/persikfloro/postmanecho/blob/main/.appveyor.yml), написан [автотест](https://github.com/persikfloro/postmanecho/blob/main/src/test/java/ru/netology/test/PostmanEchoTest.java) по схеме Given - When - Then и настроен бейдж для отображения статуса сборки
